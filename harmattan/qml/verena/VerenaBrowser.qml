			...
			if(source === "youku"){
				...
				if(option === "play"){
					...
					if(settingsObject.defaultPlayer === 0){
						...
						pageStack.push(page, {source:source, videoid:vid}, true);
					}else if(settingsObject.defaultPlayer === 1){
						Parser.loadSource(vid, source, vplayer);
					}
				}else if(option === "copy"){
					Parser.loadSource(vid, source, qobj);
				}else{
		...
		Row{
			...
			TextField{
				...
				platformSipAttributes:SipAttributes {
					...
					actionKeyEnabled: !textfield.readOnly && textfield.text.length !== 0;
