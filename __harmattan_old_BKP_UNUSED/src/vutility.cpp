	...
	engine(engine)
{
	...
	initSettingMap.insert("browser_autoload_image", QVariant(true));
	...
	initSettingMap.insert("browser_helper", QVariant(false));
...
void VUT::setUserAgent(){
	...
	if(engine)
	{
		...
		{
			static_cast<VNetworkAccessManager *>(engine -> networkAccessManager()) -> setUserAgent(userAgentMap[ua]);
		}
	}
}
